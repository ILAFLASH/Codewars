...
public class MexicanWave {
    ...
    public static String[] wave(String str) {
        int indexToPaste = 0;
        String[] resultWave = new String[str.replaceAll(" ", "").length()];
        char[] chars = str.toCharArray();

        for (int i = 0; i < str.length(); i++) {
            if (chars[i] != ' ') {
                resultWave[indexToPaste]="";
                for (int j = 0; j < str.length(); j++) {
                    if (i == j) {
                        resultWave[indexToPaste] += Character.toUpperCase(chars[j]);
                    } else {
                        resultWave[indexToPaste] += chars[j];
                    }
                }
                indexToPaste++;
            } else continue;
