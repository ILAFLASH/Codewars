...
public class ProdFib {
    public static long[] productFib(long prod) {
        List<Long> listFid = new ArrayList<>();
        listFid.add(0L);
        listFid.add(1L);
        int i = 1;
        while (listFid.get(i - 1) * listFid.get(i) < prod) {
            listFid.add(listFid.get(i - 1) + listFid.get(i));
            i++;
        }
        if (listFid.get(i - 1) * listFid.get(i) == prod) return new long[]{listFid.get(i - 1), listFid.get(i), 1L};
        else return new long[]{listFid.get(i - 1), listFid.get(i), 0L};
    }
